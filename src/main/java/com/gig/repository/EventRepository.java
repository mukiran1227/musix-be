...
public interface EventRepository extends JpaRepository<Events, UUID> {
    ...
    @Query(value = "SELECT * FROM events WHERE category = :category AND is_deleted = false AND id NOT IN(:eventId) ORDER BY start_date_time desc limit :limit offset :offset", nativeQuery = true)
    List<Events> findByCategoryAndIsDeletedFalse(@Param("category") String category, @Param("offset") int offset, @Param("limit") int limit, @Param("eventId") String eventId);
